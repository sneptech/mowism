<purpose>
Create all phases necessary to close gaps identified by `/mow:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/mow:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/mow:audit-milestone` first.
```
...
PHASES=$(node /home/max/.claude/mowism/bin/mow-tools.cjs phases list)
...
```bash
node /home/max/.claude/mowism/bin/mow-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/mow:execute-phase {N}` — if plans already exist
...
`/mow:audit-milestone` — re-audit to verify gaps closed
`/mow:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/mow:plan-phase` next
</success_criteria>
