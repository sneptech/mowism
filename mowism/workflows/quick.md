<purpose>
Execute small, ad-hoc tasks with MOW guarantees (atomic commits, STATE.md tracking). Quick mode spawns mow-planner (quick mode) + mow-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/mow:new-project` first.
...
Spawn mow-executor with plan reference:
...
MOW > QUICK TASK COMPLETE (FULL MODE)
