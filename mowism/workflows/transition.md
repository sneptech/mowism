...
**Delegate ROADMAP.md and STATE.md updates to mow-tools:**

```bash
TRANSITION=$(node /home/max/.claude/mowism/bin/mow-tools.cjs phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `mow-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node /home/max/.claude/mowism/bin/mow-tools.cjs progress bar --raw)
```
...
**Use the transition result from `mow-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/mow:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/mow:discuss-phase [X+1] --auto")
...
`/mow:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/mow:plan-phase [X+1]` — skip discussion, plan directly
- `/mow:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/mow:discuss-phase [X+1]` — revisit context
- `/mow:research-phase [X+1]` — investigate unknowns
...
```bash
node /home/max/.claude/mowism/bin/mow-tools.cjs config-set workflow.auto_advance false
```
...
Exit skill and invoke SlashCommand("/mow:complete-milestone {version}")
