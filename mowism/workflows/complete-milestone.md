...
2. **Run audit first** — `/mow:audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node /home/max/.claude/mowism/bin/mow-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `mow-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to mow-tools:**

```bash
ARCHIVE=$(node /home/max/.claude/mowism/bin/mow-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/mow:cleanup` later to archive retroactively.
...
```bash
node /home/max/.claude/mowism/bin/mow-tools.cjs commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/mow:new-milestone)
