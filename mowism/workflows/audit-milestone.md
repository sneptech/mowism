...
```bash
INIT=$(node /home/max/.claude/mowism/bin/mow-tools.cjs init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node /home/max/.claude/mowism/bin/mow-tools.cjs resolve-model mow-integration-checker --raw)
```
...
node /home/max/.claude/mowism/bin/mow-tools.cjs phases list
```
...
PHASE_INFO=$(node /home/max/.claude/mowism/bin/mow-tools.cjs find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="mow-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node /home/max/.claude/mowism/bin/mow-tools.cjs summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
- /mow:complete-milestone {version} — proceed anyway (accept tech debt)
