...
```bash
INIT=$(node ~/.claude/mowism/bin/mow-tools.cjs init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node ~/.claude/mowism/bin/mow-tools.cjs resolve-model mow-integration-checker --raw)
```
...
node ~/.claude/mowism/bin/mow-tools.cjs phases list
```
...
PHASE_INFO=$(node ~/.claude/mowism/bin/mow-tools.cjs find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/mowism/bin/mow-tools.cjs summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
