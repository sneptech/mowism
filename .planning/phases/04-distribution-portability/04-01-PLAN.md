...
must_haves:
  truths:
    - "Zero occurrences of /home/max/.claude/ in any file under agents/, commands/, mowism/, or .planning/"
...
<task type="auto">
  ...
  <action>
    ...
    This replaces ALL 411 occurrences across all 107 files in a single pass. The `sed` delimiter is `|` to avoid escaping slashes. The escaped `\.` ensures only literal dots match.
...
<verification>
```bash
# Final verification: absolute zero occurrences of the hardcoded path
grep -r '/home/max/' . --include='*.md' --include='*.cjs' --include='*.sh' | grep -v '.git/'
# Expected: no output
...
<success_criteria>
1. Zero occurrences of `/home/max/.claude/` in any file in the repository
