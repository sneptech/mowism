...
must_haves:
  truths:
    - "Zero occurrences of /home/max/ anywhere in the repository (excluding .git/)"
...
<task type="auto">
  ...
  <action>
    ...
    **Criterion 1: No hardcoded paths remaining**
    ```bash
    # Must return 0 lines
    ...
    **Criterion 4: Zero /home/max/.claude/ in distributed files**
    ```bash
    # Broader sweep: check ALL file types, not just md/cjs/sh
    grep -r '/home/max/' . | grep -v '.git/' | grep -v 'node_modules/' | wc -l
    ```
    PASS if count = 0. This catches any file type we might have missed.
...
<verification>
```bash
# The ultimate validation: zero hardcoded paths + passing tests
grep -r '/home/max/' . | grep -v '.git/' | grep -v 'node_modules/' | wc -l
# Expected: 0
