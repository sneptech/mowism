...
must_haves:
  ...
  key_links:
    ...
    - from: "~/.claude/mowism/workflows/execute-phase.md"
      to: "bin/mow-tools.cjs worktree update-plan"
      via: "node mow-tools.cjs worktree update-plan in plan execution loop"
      pattern: "worktree update-plan"
    - from: "~/.claude/mowism/workflows/execute-phase.md"
...
**2. Add auto-configure logic to mow-tools.cjs** (builds on mow-tools.cjs extended in Plan 02-01 -- wave ordering ensures 02-01 completes first) -- add a `ensureWtConfig()` function that:
...
**2. In the plan execution loop** (where each plan is executed), add a worktree plan progress update:
```
...
```
- This updates the "Plan" column in the Worktree Assignments table as execution advances through plans
- Called before each plan's execution begins, so `/mow:progress` and `/mow:worktree-status` show the current plan

**3. In the `update_roadmap` step** (after successful phase completion), add worktree release:
```
...
**4. In the `offer_next` step**, after the completion report, add a suggestion to run refine-phase:
```
---
...
**5. In the `failure_handling` section**, add a note about claim release on failure:
```
