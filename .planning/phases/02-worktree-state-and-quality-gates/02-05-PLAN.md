...
<task type="auto">
  ...
  <files>bin/mow-tools.cjs, bin/mow-tools.test.cjs, .config/wt.toml, ~/.claude/mowism/workflows/execute-phase.md, ~/.claude/mowism/workflows/progress.md, commands/mow/worktree-status.md, ~/.claude/commands/mow/worktree-status.md</files>
  <action>
Run the following validation checks. Fixes MUST be limited to the files listed above — do not modify files outside this set.
...
**Worktree claim visibility (locked decisions):**
14. `grep "Worktree Assignments" ~/.claude/mowism/workflows/progress.md` -- must find the summary section in /mow:progress
15. `grep "worktree status" ~/.claude/mowism/workflows/progress.md` -- must find the mow-tools.cjs call
16. `cat commands/mow/worktree-status.md | head -5` -- must show correct mow:worktree-status frontmatter
17. `diff commands/mow/worktree-status.md ~/.claude/commands/mow/worktree-status.md` -- must be in sync

**Test suite:**
18. `node bin/mow-tools.test.cjs 2>&1 | tail -3` -- all tests must pass

If any check fails, fix the issue in the relevant file (within the bounding set above) and re-verify.
  </action>
  <verify>All 18 checks pass. Test suite shows all tests passing.</verify>
...
<task type="auto">
  ...
  <files>commands/mow/refine-phase.md, ~/.claude/commands/mow/refine-phase.md, ~/.claude/mowism/workflows/refine-phase.md, bin/mow-tools.cjs</files>
  <action>
Run the following validation checks. Fixes MUST be limited to the files listed above — do not modify files outside this set.
