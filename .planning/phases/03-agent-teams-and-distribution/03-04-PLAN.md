...
must_haves:
  truths:
    ...
    - "/mow:team-status ??? opens the team-status help file in $EDITOR via /mow:help-open"
  artifacts:
    ...
    - path: "commands/mow/team-status.md"
      provides: "Team status display command with ??? detection"
  ...
  key_links:
    ...
    - from: "commands/mow/team-status.md"
      ...
      via: "??? detection routes to /mow:help-open team-status"
      pattern: "\\?\\?\\?"
---
...
<task type="auto">
  ...
  **??? Help Detection (MUST be first after frontmatter):**
  Add the standard ??? detection block immediately after the frontmatter closing `---`, BEFORE the command body:
  ...
  **Command body (after ??? detection):**
  ...
  <verify>Read commands/mow/team-status.md and confirm it has proper frontmatter, the ??? detection block before the command body, calls mow-tools.cjs team-status, and handles both active and inactive team states.</verify>
  <done>/mow:team-status command exists with ??? detection (routes to /mow:help-open team-status), reads team status from mow-tools.cjs, displays formatted teammate table when active, and shows helpful suggestion when no team is running.</done>
</task>
...
- commands/mow/team-status.md exists with proper frontmatter, ??? detection block, and team status display logic
- commands/mow/team-status.md ??? detection routes to `/mow:help-open team-status` before normal execution
...
- /mow:team-status ??? opens help file in $EDITOR (DIST-02 compliance for this new command)
