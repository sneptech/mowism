...
must_haves:
  truths:
    ...
    - "Phase workers send the 6 new checkpoint message types at defined milestones"
  ...
  artifacts:
    - path: "agents/mow-team-lead.md"
      provides: "Dashboard rendering integration and extended message handling for 13 event types"
      contains: "dashboard render"
    - path: "agents/mow-phase-worker.md"
      provides: "Banner printing, extended milestone messaging with 11 checkpoint types, input wait confirmation, context recap"
  ...
  key_links:
    ...
    - from: "agents/mow-phase-worker.md"
      ...
      via: "Worker sends new checkpoint types (task_claimed, commit_made, task_complete, stage_transition, input_needed, plan_created)"
      pattern: "message format (task_claimed|commit_made|task_complete|stage_transition|input_needed|plan_created)"
    - from: "agents/mow-phase-worker.md"
...
<task type="auto">
  ...
  **3. Expand the message processing event type table:**
  ...
  Add rows for the 6 new v2 event types:
  ...
  **6. Add dashboard cleanup to single-phase completion:**
  ...
  4. All 13 event types are listed in the message processing table
  ...
  <done>Team lead renders dashboard after every worker message, displays input routing via pinned notifications, prints orchestrator banner at startup, handles all 13 event types, cleans up dashboard state on shutdown.</done>
</task>
...
<task type="auto">
  ...
  **2. Expand the messaging protocol with new checkpoint types:**
  ...
  In `<messaging_protocol>`, expand the milestone table from 5 to 11 checkpoint types (5 existing + 6 new):
  ...
  **7. Add error state banner:**
  ...
  2. All 11 checkpoint message types documented in messaging protocol table
  ...
  <done>Phase worker prints startup banner, sends all 11 checkpoint message types at correct lifecycle milestones, includes activity descriptions, confirms input waits, prints context recap on resume, echoes phase context for permission prompts, and switches to caution stripe banner on error.</done>
</task>
...
2. mow-team-lead.md handles all 13 event types (7 existing + 6 new)
3. mow-phase-worker.md prints phase banner at startup
4. mow-phase-worker.md sends task_claimed, commit_made, task_complete, stage_transition, input_needed, and plan_created at correct lifecycle points
...
- Workers print phase-colored banner at startup, send 11 checkpoint types with activity descriptions
