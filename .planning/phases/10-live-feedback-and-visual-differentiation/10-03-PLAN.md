...
must_haves:
  ...
  artifacts:
    ...
    - path: "agents/mow-phase-worker.md"
      provides: "Banner printing, extended milestone messaging with 10 checkpoint types, input wait confirmation, context recap"
...
<task type="auto">
  ...
  **2. Expand the messaging protocol with new checkpoint types:**
  ...
  In `<messaging_protocol>`, expand the milestone table from 5 to 10 checkpoint types (5 existing + 5 new):
  ...
  **7. Add error state banner:**
  ...
  2. All 10 checkpoint message types documented in messaging protocol table
  ...
  <done>Phase worker prints startup banner, sends all 10 checkpoint message types at correct lifecycle milestones, includes activity descriptions, confirms input waits, prints context recap on resume, echoes phase context for permission prompts, and switches to caution stripe banner on error.</done>
</task>
...
- Workers print phase-colored banner at startup, send 10 checkpoint types with activity descriptions
