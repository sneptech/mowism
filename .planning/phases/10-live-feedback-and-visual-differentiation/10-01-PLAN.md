...
must_haves:
  truths:
    - "Message schema v2 validates all 12 event types (7 existing + 5 new checkpoint types)"
  ...
  key_links:
    - from: "bin/mow-tools.cjs"
      ...
      via: "Schema v2 adds task_claimed, commit_made, stage_transition, input_needed, plan_created"
      pattern: "task_claimed.*commit_made.*stage_transition.*input_needed.*plan_created"
    - from: "bin/mow-tools.cjs"
      to: "PHASE_PALETTE"
...
<objective>
Implement the visual and messaging foundation for Phase 10: ANSI 256-color helper functions, curated phase color palette, message schema v2 with 5 new checkpoint types, full-width banner renderer for orchestrator/workers, progress bar renderer with active/blocked states, and caution stripe error banner.
...
<task type="auto">
  ...
  **4. Extend message schema to v2:**
  ...
  - Add 5 new entries to `MESSAGE_REQUIRED_FIELDS`:
  ...
  - Add all 5 new types to the `ENABLED_EVENTS` array
  ...
  **5. Add CONFIG_DEFAULTS for feedback settings:**
  ...
  <done>MESSAGE_SCHEMA_VERSION is 2, all 12 event types validate, 256-color helpers exist, phase palette returns correct colors, INPUT_TYPES constant exists, activity field handled in format/parse, feedback config defaults added.</done>
</task>
...
<task type="auto">
  ...
  **Message schema v2 tests (6 tests):**
  ...
  3. `message format input_needed` validates input_type against INPUT_TYPES (warn on unknown, still produce output)
  4. `message format plan_created` includes phase and plan
  5. `message parse` accepts v2 messages with new types
  6. `message format` with `--activity "Implementing dashboard"` includes activity field truncated to 40 chars

  **Color helper tests (4 tests):**
  7. `format banner` with FORCE_COLOR=1 produces ANSI escape codes in output
  8. `format banner` with NO_COLOR=1 produces plain text (no escape codes)
  9. `format banner-phase --phase 10` uses phase palette color (not red)
  10. `format progress --percent 50 --width 10` produces bar with 5 fill chars and 5 empty chars

  **Caution stripe test (1 test):**
  11. `format banner-error` with FORCE_COLOR=1 contains warning symbol `\u26A0` and yellow ANSI code `48;5;226`
  ...
  <verify>
  Run `node bin/mow-tools.test.cjs 2>&1 | tail -5` -- all tests pass, including the 11 new tests.
  Check test count increased by 11 from baseline.
  </verify>
  <done>11 new tests pass covering message schema v2, color helpers, banner rendering, progress bar, and caution stripe. Zero regressions on existing tests.</done>
</task>
...
- All 12 event types (7 old + 5 new) validate in format and parse
