...
<task type="auto">
  ...
  For config defaults: in the `loadConfig()` function, ensure the default config object includes:
  - `worker: { stage_gates: 'none' }` — default to fully autonomous after discuss
  - `workflow.verifier` already exists; verify it defaults to `true`. If not present in defaults, add it.

  These are simple property additions — no new functions, no new commands.
  </action>
  <verify>
  ```bash
  ...
  node bin/mow-tools.cjs config-get worker.stage_gates 2>/dev/null || echo "config-get should return none"
  ```
  </verify>
