...
gaps:
  ...
  integration:
    - id: "hardcoded-paths"
      ...
      description: "31/33 workflow files, 32/34 command files, and 9/12 agent files use ~/.claude/ as hardcoded paths. install.sh copies these without substitution. Product only works on developer's machine."
...
31/33 workflow files, 32/34 command files, and 9/12 agent files contain hardcoded `~/.claude/` paths. `bin/install.sh` copies these files without path substitution. On any machine other than the developer's, every `mow-tools.cjs` invocation fails with "No such file or directory".
